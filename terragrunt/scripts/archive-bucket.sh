...
include "envcommon" {
  path = "\${dirname(find_in_parent_folders())}/_envcommon/global/s3-archive.hcl"
}
...
# Function to source get-tf-registry-token-manual.sh script
source_get_tf_registry_token() {
    # Get region from user input
    read -p "Enter the region for sourcing get-tf-registry-token-manual.sh script: " region
    source ../../../scripts/get-tf-registry-token-manual.sh "$region"
}

# Function to apply Terragrunt
apply_terragrunt() {
    # Apply Terragrunt
    terragrunt apply
}

# Function to create folder, Terragrunt file, source get-tf-registry-token script, and apply Terragrunt
execute_all() {
    create_folder
    create_terragrunt_file
    source_get_tf_registry_token
...
# Call the function to execute all tasks
execute_all
